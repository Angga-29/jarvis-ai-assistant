...
def _extract_text_and_images(content) -> tuple[str, list[str] | None]:
    """Split an OpenAI-style content field into (text, images).

    ``content`` is either a plain string (the common case) or a list of
    content-part dicts, e.g.:
        [{"type": "text", "text": "..."},
         {"type": "image_url", "image_url": {"url": "data:image/png;base64,...."}}]
    Images are returned as bare base64 strings (Ollama's /api/chat "images"
    field does not want the "data:image/png;base64," prefix).
    """
    ...
    for part in content or []:
        ...
        if ptype == "text":
            ...
            if url:
                images.append(url)
    return " ".join(text_parts), (images or None)
...
async def chat_completions(request_body: ChatCompletionRequest, request: Request):
    ...
    # Vision requests (content is a list with an image_url part) can't go
    # through the agent — agent.run() in _handle_agent only accepts a plain
    # string and would crash forwarding a list straight to Ollama's API
    # ("cannot unmarshal array into ... content of type string"). Force
    # these through the direct engine path, which _to_messages() already
    # knows how to split into text + images.
    _last_content = request_body.messages[-1].content if request_body.messages else ""
    _has_image = isinstance(_last_content, list) and any(
        isinstance(p, dict) and p.get("type") == "image_url" for p in _last_content
    )
    if agent is not None and not request_body.tools and not _has_image:
